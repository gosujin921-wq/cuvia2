...
import { getEventsByDomain, getEventById, generateAIInsight, domainLabels } from '@/lib/events-data';
...
현재 3개 CCTV가 활발히 추적 중이며, 추가 2개 CCTV를 예방적으로 모니터링하는 것을 권장합니다.`;
  ...
  const selectedBaseEvent = selectedEvent?.id ? getEventById(selectedEvent.id) : undefined;
  const categoryLabel = selectedBaseEvent ? domainLabels[selectedBaseEvent.domain] : domainLabels.A;
  const aiSummary = selectedBaseEvent ? generateAIInsight(selectedBaseEvent) : '선택된 사건을 불러오는 중입니다.';
  const detailStats = [
    { label: '위험도', value: selectedEvent?.risk },
    { label: '위험도 수치', value: selectedEvent ? `${selectedEvent.pScore}%` : undefined },
    { label: '진행 상태', value: selectedBaseEvent?.status ?? selectedEvent?.status },
    { label: '신고 기관', value: selectedEvent?.source },
    { label: '발생 시간', value: selectedEvent?.time },
  ].filter((item) => item.value);

  const buildRiskFactors = (event: CrimeEvent | null, base?: ReturnType<typeof getEventById>) => {
    if (!event) return [] as RiskFactor[];
    const factors: RiskFactor[] = [];
    
    // 이벤트 ID 기반 구체적인 위험 요인 분석
    if (event.id.includes('003') || (event.type.includes('차량도주') || event.type.includes('용의차량'))) {
      // 차량도주 사건 (예: A-20241124-003)
      factors.push(
        ...
        { label: '오토바이 특성', value: '무등록 번호판 / 검정색 풀페이스 헬멧', reason: '신원 식별 난이도 높음', level: 'medium' },
        { label: 'ReID 일치율', value: `${event.pScore}%`, reason: '은행 강도 CCTV 피의자와 외형·체형 유사', level: 'medium' }
      );
      ...
      factors.push(
        ...
        { label: 'ReID 일치율', value: `${event.pScore}%`, reason: '흉기 소지 사건과 동일 인물 78%', level: 'medium' },
        { label: '시간대', value: `야간(${event.time})`, reason: '야간 폭행은 중상 가능성 높음', level: 'high' }
      );
    } else if (event.type.includes('절도') || event.type.includes('강도') || event.id.includes('002')) {
      // 절도 사건
      factors.push(
        { label: '행동 패턴', value: '현금 절취 포착', reason: '가방에서 현금 다발을 넣는 장면 명확히 확인', level: 'high' },
        { label: '동선 패턴', value: '동선 반복 3회', reason: '의도적 절도 행위로 판단', level: 'high' },
        { label: 'CCTV 포착', value: 'CCTV-7, CCTV-12', reason: '연속 포착으로 확증 높음', level: 'medium' },
        { label: '시간대', value: event.time, reason: '상가 영업 시간대, 피해 확산 가능', level: 'medium' },
        { label: '연관 이벤트', value: '유사 절도 패턴 2건', reason: '동일 장소 유사 사건 이력', level: 'medium' },
        { label: '위험도 수치', value: `${event.pScore}%`, reason: 'AI 감지 신뢰도 높음', level: 'medium' }
      );
    } else if (event.type.includes('실종') || event.type.includes('미아') || event.id.includes('005')) {
      // 실종 사건
      factors.push(
        { label: '연령', value: '8세', reason: '저연령 실종은 즉시 대응 대상', level: 'high' },
        { label: '경과 시간', value: '30분', reason: '골든타임(10분) 경과, 위험도 급증', level: 'high' },
        { label: '연관 이벤트', value: '배회 감지 연관 76%', reason: '동일 방향·거리 15m 이내', level: 'strong' },
        { label: 'ReID 일치율', value: `${event.pScore}%`, reason: '최근 실종자 CCTV와 일치', level: 'medium' },
        { label: '위치', value: base?.location ?? '확인 중', reason: '공원 인근, 유동 인구 적음', level: 'medium' },
        { label: '시간대', value: event.time, reason: '야간 시간대, 시야 확보 어려움', level: 'high' }
      );
    } else if (event.type.includes('위험행동') || event.type.includes('흉기') || event.id.includes('006')) {
      // 위험행동 사건
      factors.push(
        { label: '행동 패턴', value: '흉기 소지 위협 행동', reason: '긴 물체 소지 및 위협 행동 포착', level: 'high' },
        { label: '시간대', value: `야간(${event.time})`, reason: '야간 흉기 소지는 즉시 대응 필요', level: 'high' },
        { label: '위치', value: base?.location ?? '확인 중', reason: '역 앞 광장, 다수 인파 노출', level: 'high' },
        { label: 'AI 감지', value: '고신뢰도', reason: 'AI 자동 감지, 위협 행동 명확', level: 'medium' },
        { label: 'ReID 일치율', value: `${event.pScore}%`, reason: '과거 유사 사건과 동일 인물 가능성', level: 'medium' },
        { label: '연관 이벤트', value: '폭행 사건 연관 의심', reason: '동일 시간대 유사 패턴', level: 'medium' }
      );
    } else if (event.type.includes('기물파손') || event.id.includes('004')) {
      // 기물파손 사건
      factors.push(
        { label: '행동 패턴', value: '음주 난동 + 기물 파손', reason: '다수 기물 파손 신고 접수', level: 'medium' },
        { label: '시간대', value: event.time, reason: '야간 시간대, 소음 민원 가능', level: 'medium' },
        { label: '위치', value: base?.location ?? '확인 중', reason: '시장로, 상가 피해 확산 가능', level: 'medium' },
        { label: '신고 건수', value: '다수 신고', reason: '여러 신고자 동시 접수', level: 'medium' },
        { label: '연관 이벤트', value: '유사 난동 이력', reason: '동일 장소 유사 사건', level: 'low' }
      );
    } else {
      // 기본 위험 요인
      factors.push(
        ...
        { label: '신고 기관', value: event.source, reason: 'AI·112 동시 감지', level: 'low' }
      );
    }
    return factors;
  };

  const riskFactors = useMemo(() => buildRiskFactors(selectedEvent, selectedBaseEvent), [selectedEvent, selectedBaseEvent]);
  ...
  return (
    <div className="flex flex-col h-screen bg-[#0f0f0f] overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Left Panel - 이벤트 상세 */}
        <aside className="w-96 flex-shrink-0 bg-[#1a1a1a] border-r border-[#2a2a2a] flex flex-col" style={{ borderWidth: '1px' }}>
          {selectedEvent ? (
            <>
              <div className="p-5 border-b border-[#2a2a2a] space-y-4" style={{ borderWidth: '1px' }}>
                <div className="space-y-1">
                  <p className="text-xs text-gray-500 tracking-tight">사건번호</p>
                  <p className="text-white font-mono text-sm">{selectedEvent.id}</p>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-400">카테고리</span>
                  <span className="text-blue-400 font-semibold">{categoryLabel}</span>
                </div>
                <div className="flex flex-wrap gap-2">
                  <span className="px-3 py-1 rounded-full text-xs bg-white/10 text-white border border-white/20" style={{ borderWidth: '1px' }}>
                    {selectedEvent.type}
                  </span>
                  <span
                    className={`px-3 py-1 rounded-full text-xs border ${
                      selectedEvent.risk === 'HIGH'
                        ...
                        : selectedEvent.risk === 'MEDIUM'
                          ...
                          : 'border-blue-500/60 text-blue-200 bg-blue-500/10'
                    }`}
                    style={{ borderWidth: '1px' }}
                  >
                    위험도 · {selectedEvent.risk}
                  </span>
                  <span className="px-3 py-1 rounded-full text-xs border border-blue-500/40 text-blue-200 bg-blue-500/10" style={{ borderWidth: '1px' }}>
                    위험도 수치 {selectedEvent.pScore}%
                  </span>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4">
                <div className="bg-[#0f0f0f] border border-[#2a2a2a] rounded-lg p-4 space-y-3" style={{ borderWidth: '1px' }}>
                ...
                <div className="bg-[#0f0f0f] border border-[#2a2a2a] rounded-lg p-4 space-y-3" style={{ borderWidth: '1px' }}>
                  <h4 className="text-white font-semibold text-sm">기본 정보</h4>
                  ...
                  <div className="bg-[#0f0f0f] border border-[#2a2a2a] rounded-lg p-4 space-y-3" style={{ borderWidth: '1px' }}>
                    ...
                    <div className="space-y-2">
                      {riskFactors.map((factor) => (
                        <div key={factor.label} className="text-sm space-y-1">
                          <div className="flex items-center justify-between">
                            <div className="text-gray-300 font-medium">{factor.label}</div>
                            <div className="flex items-center gap-2">
                              <span className="text-white font-semibold">{factor.value}</span>
                              {factor.level === 'strong' ? (
                                <span className="text-red-500 text-xs font-bold">‼️ 강연관</span>
                              ) : factor.level === 'high' ? (
                                <span className="text-red-400 text-xs">⚠️ 위험</span>
                              ) : factor.level === 'medium' ? (
                                <span className="text-yellow-400 text-xs">⚠️ 주의</span>
                              ) : (
                                <span className="text-gray-500 text-xs">ℹ️ 정보</span>
                              )}
                            </div>
                          </div>
                          <p className="text-gray-500 text-xs leading-relaxed pl-4">
                            → {factor.reason}
                          </p>
                        </div>
                      ))}
                ...
                {selectedEvent.description && (
                  <div className="bg-[#0f0f0f] border border-[#2a2a2a] rounded-lg p-4 space-y-3" style={{ borderWidth: '1px' }}>
                    <h4 className="text-white font-semibold text-sm">상황 설명</h4>
