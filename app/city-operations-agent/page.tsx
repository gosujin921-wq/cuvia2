...
// 공통 데이터에서 도시 운영(F) 이벤트만 필터링
const getCityOperationsEvents = (): DisasterEvent[] => {
  return getEventsByDomain('F').map((event) => ({
    id: event.eventId,
    type: event.type,
    title: event.title,
    time: event.time,
...
const CityOperationsAgentPageContent = () => {
  const searchParams = useSearchParams();
  const events = useMemo(() => getCityOperationsEvents(), []);
  ...
  useEffect(() => {
    ...
    if (eventId) {
      ...
      if (baseEvent && baseEvent.domain === 'F') {
      ...
      <header className="flex h-16 items-center justify-between bg-[#1a1a1a] border-b border-[#2a2a2a] px-6" style={{ borderWidth: '1px' }}>
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <Icon icon="mdi:city" className="w-6 h-6 text-white" />
            <span className="text-xl font-semibold text-white">도시 운영 · 환경 Agent</span>
          </div>
        </div>
        <div className="flex items-center gap-3">
              ...
              {filteredEvents.map((event) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-blue-400 text-xs">{domainLabels.F}</span>
                  </div>
...
export default function CityOperationsAgentPage() {
  return (
    <Suspense fallback={
      ...
      <CityOperationsAgentPageContent />
    </Suspense>
  );
}
